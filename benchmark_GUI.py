...
#
"""
    List of all function for quick search via Ctrl-F
...
# Tab "sql_query"
    ...
    def button_execute_sql_query_OnButton()

# Tab "Tests"
    def button_regex_OnButton()
    def button_add_map_OnButton()
    def button_reset_maps_OnButton()
    def button_add_10_lines_OnButton()
    def button_edit_file_list_OnButton()
    def button_set_the_path_OnButton()

    def button_create_test_run_script_OnButton()
    def button_load_script_OnButton()

    def get_description()

    def button_start_test_OnButton()
    def get_arguments_to_start_test()
    def start_test()
    def m_timer_OnTimer()

# Tab "Results"
    def button_save_report_OnButton()
    def button_tests_update_OnButton()
    def button_update_benchmark_results_OnButton()
    def list_ctrl_tests_COL_CLICK()
    def list_ctrl_tests_OnRightClick()
    def list_ctrl_tests_SELECTED()
    def list_ctrl_benchmark_results_COL_CLICK()
    def list_ctrl_benchmark_results_SELECTED()

    def change_description()
    def update_tests_results()
    def update_benchmark_results()
    def list_get_text_for_column()
...
class MainFrame(wx.Frame):  # type: ignore
    def OnClose(self, event: Any) -> None:
        """when closing, save the settings"""
        self.save_settings()
        self.Destroy()
    ...
    def get_list_settings(self, list: Any) -> dict[str, bool]:
        """Auxiliary function for save_settings(self)"""
        """ returns a dictionary of the form {"column name": True} """
        result = {}
    ...
    def m_timer_OnTimer(self, event: Any) -> None:
        """the timer turns on after starting start_test()"""
        """ viewing the file "self.temporary_file" and output new lines from it """
        """ (the output from the start_test() function is redirected to the file) """
            ...
            with open(script) as f:
                for line in f.readlines():
                    if "# settings=" in line:
                        ...
                        self.SetTitle(f"benchmark GUI - {script}")
                        break
        event.Skip()
    ...
    def button_create_test_run_script_OnButton(
        ...
        if saveFileDialog.ShowModal() == wx.ID_OK:
            ...
            self.SetTitle(f"benchmark GUI - {filename}")
        event.Skip()
    ...
    def start_test(
        ...
        """performs a benchmark, executed in another thread"""
